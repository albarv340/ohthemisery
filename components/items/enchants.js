...
export default function Enchants(data) {
    ...
    enchants.push(item.Agility ? <span className={styles[enchantStyle(item.Agility, "Agility")]} key='Agility'>{item.Agility[0] == "-" ? "" : "+"}{item.Agility} Agility</span> : "")
    enchants.push(item.Armor ? <span className={styles[enchantStyle(item.Armor, "Armor")]} key='Armor'>{item.Armor[0] == "-" ? "" : "+"}{item.Armor} Armor</span> : "")
    if (item["Void Tether"]) {
